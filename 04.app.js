...
const moment = require('moment'); //moment 가져오기
...
/*************Ejs Init****************/
//app.set(변수명, 값);
app.set('view engine', 'ejs'); //앞으로 html을 동적으로 만드는 일을 pug가 함 view engine으로 pug를 쓰겠다.
app.set('views', path.join(__dirname, './views')); //views 위치를 알려줌
app.locals.pretty = true; //브라우저에 보내주는 html을 이쁘게


/*************req.body Init****************/
app.use(express.json()); //들어오는 명령 json으로 변경
app.use(express.urlencoded({extended: false})); //post방식을 받기
...
const gbook = [
  {id: 2, content: '좋아요!', createdAt: '2021-05-25 18:13:15'},
  {id: 1, content: '좋아!', createdAt: '2021-05-25 04:08:15'},
];
/*************Rounter Static****************/
...
app.get('/event', (req, res, next) =>{
  ...
  res.render('event/event.ejs', {event, pageTitle: '이벤트 사이트'});
});
